...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # output, (h_enc, c_enc) = self.encoder(nn.utils.rnn.pack_padded_sequence(X, source_lengths))
        # enc_hiddens, sequence_length = nn.utils.rnn.pad_packed_sequence(output, batch_first = True)
